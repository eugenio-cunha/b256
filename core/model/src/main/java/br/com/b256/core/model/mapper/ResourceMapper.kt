 ...
 * ao valor de [Resource.Success]. Se o [Resource] atual for [Resource.Failure] ou [Resource.Loading],
...
inline fun <T, R> Resource<T>.map(transform: (T?) -> R): Resource<R> = when (this) {
    ...
    is Resource.Failure -> Resource.Failure(message = message)
 ...
 * emitido pelo Flow original, ou [Resource.Failure] se ocorrer uma exceção durante a coleta do Flow.
 *
...
fun <T> Flow<T>.asResource(): Flow<Resource<T>> =
    map<T, Resource<T>> { Resource.Success(data = it) }
        ...
        .catch { emit(value = Resource.Failure(message = it.localizedMessage)) }
